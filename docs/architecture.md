...
## Components
- **API layer (`src/app/api`)**: FastAPI routers for task operations (`/tasks`) and admin controls (`/admin`). Input validation relies on Pydantic models.
- **Service layer (`src/app/services`)**: Contains the `StateStore` (Redis persistence), `TaskExecutor` (async execution and state transitions), and `AdminService` (priority and blocking controls).
- **Core utilities (`src/app/core`)**: Environment-driven settings, Redis client abstraction for read/write hosts, and logging configuration suited for container stdout/stderr.
- **Models (`src/app/models`)**: Task payloads, status enums, and persisted task state with timestamps, logs, priority, and results.
...
- Add new admin features by extending `AdminService` and wiring routes in `src/app/api/admin.py`.
